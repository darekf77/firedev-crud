...
export class DbCrud {
  ...
  public controllersInstances: (Models.db.BaseController<any>)[] = [];
  ...
  //#region api / get all
  async getAll<T extends Models.db.DBBaseEntity>(classFN: Function): Promise<T[]> {
    //#region @backendFunc
    const entityName: Models.db.EntityNames = Models.db.getEntityNameByClassFN(classFN);
  ...
  async addIfNotExist(entity: Models.db.DBBaseEntity): Promise<boolean> {
    //#region @backendFunc
  ...
  //#region api / remove
  async remove(entity: Models.db.DBBaseEntity): Promise<boolean> {
    //#region @backendFunc
    const classFN = CLASS.getFromObject(entity)
    const all = await this.getAll(CLASS.getFromObject(entity))
  ...
  //#region api / set
  async set(entity: Models.db.DBBaseEntity) {
    //#region @backend
    const classFN = CLASS.getFromObject(entity)
  ...
  //#region api / set buld
  async setBulk(entites: Models.db.DBBaseEntity[], classFN: Function): Promise<boolean> {
    //#region @backendFunc
  ...
  //#region methods / after retrive
  protected async afterRetrive<T = any>(value: any, entityName: Models.db.EntityNames)
    : Promise<Models.db.DBBaseEntity> {
    //#region @backendFunc
    ...
    for (let index = 0; index < entites.length; index++) {
      ...
      if (entityName === Models.db.getEntityNameByClassFN(entityClassFn)) {
        // @ts-ignore
        const v = new (entityClassFn as any)(value) as Models.db.DBBaseEntity<any>;
  ...
  //#region methods / prepare entity for save
  protected async preprareEntityForSave(entity: Models.db.DBBaseEntity) {
    //#region @backendFunc
